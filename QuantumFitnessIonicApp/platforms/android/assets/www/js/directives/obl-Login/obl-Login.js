				...
				controller: ['$scope','$http','$state','$stateParams','$ionicPopup','constants','$auth','$rootScope',
					function($scope,$http,$state,$stateParams,$ionicPopup,constants,$auth,$rootScope) {
						...
						$scope.login = function() {
							...
							$http.post("http://192.168.8.156:1337"+"/templatesAuth/authenticateForApp",data)
								.then(function(res){
										var requestParams = {
										...
										if ($scope.parentobj.appUserId!=='unknown'&& $scope.parentobj.appUserId!==res.data.user.sub ){
                          $rootScope.cart.cartSize = 0;
                          $rootScope.cart.cartItems = [];
                          $rootScope.parentobj.cartSize = 0;

                    }else if ($scope.parentobj.appUserId==='unknown'){
                      $scope.parentobj.appUserId = res.data.user.sub;
                    }
