...
mobileApp.controller('cartCtrl', function ($scope, $rootScope, $http, $state, $stateParams, $ionicPopup, constants, PaypalService, $log, $ionicNavBarDelegate, $location, $timeout, $ionicLoading) {
    ...
    $scope.$on('$ionicView.enter', function () {
        console.log($rootScope.deliveryParams);
        if ($rootScope.deliveryParams) {
            if ($rootScope.deliveryParams.email) {
                $scope.details.email = $rootScope.deliveryParams.email;
                $scope.details.city = $rootScope.deliveryParams.city;
                $scope.details.location = $rootScope.deliveryParams.location;
                $scope.details.method = $rootScope.deliveryParams.methos;
                $scope.details.name = $rootScope.deliveryParams.name;
                $scope.details.phone = $rootScope.deliveryParams.phone;
                $scope.details.province = $rootScope.deliveryParams.province;
                $scope.details.streetName = $rootScope.deliveryParams.streetName;
                $scope.details.streetNumber = $rootScope.deliveryParams.streetNumber;
                $scope.details.zip = $rootScope.deliveryParams.zip;

                $scope.dpCities = $rootScope.paramCities;
            }
        }
    });
    ...
    if ($scope.user) {
        ...
        // $scope.details.province = $scope.user.province;
        // $scope.details.city = $scope.user.city;
    ...
    $scope.selectedProvince = function (data) {
        console.log($scope.details.province);
        ...
        $rootScope.paramCities = $scope.dpCities;
    }
    //get the privinces
    $http.get(constants.SERVER_URL + '/get/provinces').success(function (data) {
        $scope.cities = data;
    ...
    $scope.deliver = function (deliverDetails) {
        $rootScope.deliveryParams = deliverDetails;
