...
module.exports = {
    ...
    sendPasswordResetEmail: function(req, res) {
        var email = req.body.email,
            ...
            url = req.body.url,
            // AppUser find query
            criteria = { email: email, appId : appId },
            // Encrypt the email
            encryptedEmail = cryptr.encrypt(email);
            ...
            Passwords.encryptPassword({
                ...
                success: function(hash) {
                    ...
                    AppUser.update(criteria, { password: password })
                        .exec(function(err,user){
                            if (err) return res.send({ message: 'error'});
                            if (user.length > 0) return res.send({ message: 'success'});
                            if (user.length == 0) return res.send({ message: 'failed'});
                    });
                }
            });
        });
